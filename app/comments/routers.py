...
@comment_router.get('/{post_id}/comments',
                    response_model=list[OutputComment],
...
def create_comment(post_id: uuid.UUID,
    ...
    if parent_id:
        parent = db.exec(select(Comments).
                         where(Comments.id == parent_id,Comments.post_id == post_id)).first()
        if not parent:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                                detail={'error':'parent_id not found'})
...
@comment_router.put('/{post_id}/comments/{comment_id}',
                    response_model=OutputComment,
...
def update_comment(post_id: uuid.UUID,
    ...
    using the post id and the comment id
    ...
    if not comment:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail={"error":"comment not found"})
    if comment.user_id != user.id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
                            detail={"error":"You are unauthorized to edit this comment"})
    data = comment_data.model_dump()
...
def delete_comment(post_id: uuid.UUID,
    ...
    using the post id and the commnet id
