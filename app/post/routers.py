...
def get_all_posts(offset: int=0,
    ...
    posts = db.exec(select(Post).order_by(Post.created_at.desc())
...
def update_post(post_id: uuid.UUID,
    ...
    """Used to update a post
       ...
       usnig the post id only the creator of the post can edit the post."""
    ...
    if post.author_id != user.id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
                            detail={"error":"You are Unauthorized to update this post"})
...
def delete_post(post_id: uuid.UUID,
    ...
    using the post id only the creator can delete the post."""
    ...
    if post.author_id != user.id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
                            detail={"error":"You are Unauthorized to delete this post"})
    try:
        db.delete(post)
        db.commit()
    except Exception as e:
